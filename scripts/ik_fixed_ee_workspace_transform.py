...
class IKCommander(Node):
    ...
    def move_to_pose(self, x_input, y_input, z):
        GROUND_Z_LIMIT = 0.0  # Set your ground level

        # ArUco-based translation
        x2 = 0.5334
        y2 = 0.3302
        x = float(x_input) - x2
        y = float(y_input) - y2
        ...
        if future.result() and future.result().error_code.val == 1:
            ...
            self.get_logger().info(f"IK successful. Moving to translated x={x:.4f}, y={y:.4f}, z={z} with fixed orientation")
...
def main(args=None):
    ...
    try:
        while True:
            inp = input("Enter target x y z (in ArUco frame, separated by spaces): ")
            if not inp.strip():
                break
